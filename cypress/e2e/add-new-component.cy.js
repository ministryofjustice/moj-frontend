describe('Add New Component Workflow', () => {
  beforeEach(() => {
    ...
    cy.intercept('POST', '/get-involved/add-new-component/component-details', (req) => {
      req.reply({
        statusCode: 302,
        headers: { location: '/get-involved/add-new-component/component-image' },
      });
    }).as('submitComponentDetails');

    cy.intercept('POST', '/get-involved/add-new-component/component-image', (req) => {
      req.reply({
        statusCode: 302,
    ...
    cy.intercept('POST', '/get-involved/add-new-component/your-details', (req) => {
      req.reply({
        statusCode: 302,
        headers: { location: '/get-involved/add-new-component/check-your-answers' },
      });
    }).as('submitYourDetails');

    // Intercept the final submission
    cy.intercept('POST', 'http://localhost:3000/get-involved/add-new-component/check-your-answers', (req) => {
      req.reply({
        statusCode: 200,
  ...
  it('completes the entire workflow with final API call', () => {
    ...
    cy.visit('http://localhost:3000/get-involved/add-new-component/component-details');
    cy.get('input#component-name').type('Accessibility Checker');
    cy.get('textarea[name="briefDescription"]').type('A tool to ensure web components meet accessibility standards.');
    cy.get('textarea[name="whyNeeded"]').type('This component is essential to improve accessibility compliance.');
    cy.get('button[type="submit"]').click();

    // Step 2: Verify redirection to the component-image page
    cy.url().should('include', '/get-involved/add-new-component/component-image');
    ...
    cy.url().should('include', '/get-involved/add-new-component/check-your-answers');
