...
test('submit a component is visible', async ({ page }) => {
  ...
  await expect(
    page
      .getByLabel('About the Design System')
      .getByRole('link', { name: 'Submit a Component' })
  ).toBeVisible()
  await expect(
    main.getByRole('link', { name: 'Submit a Component' })
  ).toBeVisible()
})
...
test.describe('navigation', () => {
  test('menus are collapsed', async ({ page }) => {
    await page.goto('/')
    await expect(
      page.getByRole('link', { name: 'Date picker' })
    ).not.toBeVisible()
    await expect(
      page.getByRole('link', { name: 'Filter a list' })
    ).not.toBeVisible()
    await expect(
      page.getByRole('link', { name: 'Use Nunjucks' })
    ).not.toBeVisible()
  })
