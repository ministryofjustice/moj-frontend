...
| itemLabel              | string  | Yes      | The singular label for each item. Should be title case. Defaults to 'Item'.                    |
...
| html                   | string  | Yes      | HTML to use/render within the fieldset element.|
| classes                | string  | No       | Classes to add to the fieldset container. |
| role                   | string  | No       | Optional ARIA role attribute. |
| attributes             | string  | No       | HTML attributes (for example data attributes) to add to the fieldset container. For the 'inline' layout, the attributes must contain an `aria-labelledby` attribute with an element ID. |
