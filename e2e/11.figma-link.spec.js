...
import { FigmaLinkPage } from './pages/figma-link-page';
...
test('empty state', async () => {
  ...
  describe('character count', async () => {
    ...
    test('remaining count visible', async () => {
      ...
      await expect(testPage.countMessage).toHaveText('You have 250 words remaining')
    })

    test('count reduces', async () => {
      await testPage.infoInput.pressSequentially('word word word ')
      await expect(testPage.countMessage).toHaveText('You have 247 words remaining')
    })
