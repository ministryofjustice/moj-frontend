...
import { ComponentImagePage } from './pages/component-image-page';
...
test('file too large', async () => {
  ...
  await testPage.expectErrorSummaryWithMessages(['The selected file must be smaller than 10MB'])
})
...
test('check your answers', async ({page}) => {
  ...
  await expect(cyaPage.componentImageCard.getByRole('link', {name: 'Change'})).toBeVisible()
  await expect(cyaPage.componentImageCard.getByRole('link', {name: 'Remove'})).not.toBeVisible()
