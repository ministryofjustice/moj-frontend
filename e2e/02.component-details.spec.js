...
import { ComponentDetailsPage } from './pages/component-details-page';
...
test('check your answers', async ({ page }) => {
  ...
  await expect(cyaPage.componentDetailsCard).toContainText('It expands and contracts')
  await expect(cyaPage.componentDetailsCard).toContainText('To hide and show content')

  await expect(cyaPage.componentDetailsCard.getByRole('link', {name: 'Change'})).toBeVisible()
  await expect(cyaPage.componentDetailsCard.getByRole('link', {name: 'Remove'})).not.toBeVisible()

  await cyaPage.componentDetailsCard.getByRole('link', {name: 'Change'}).click()
