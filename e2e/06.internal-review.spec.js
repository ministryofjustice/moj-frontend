...
import { InternalReviewPage } from './pages/internal-review-page'
...
let accessibilityPage
  ...
  test('check your answers', async ({page}) => {
    ...
    await expect(cyaPage.internalReviewCard.getByRole('link', {name: 'Change'})).toBeVisible()
    await expect(cyaPage.internalReviewCard.getByRole('link', {name: 'Remove'})).not.toBeVisible()

    // Change and complete external audit section
    await cyaPage.internalReviewCard.getByRole('link', {name: 'Change'}).click()
    ...
    await expect(cyaPage.internalReviewCard).toContainText(`${dayjs().format('D MMMM YYYY')}`)
    await expect(cyaPage.internalReviewCard).toContainText('Many issues')
    await expect(cyaPage.internalReviewCard.getByRole('link', {name: 'Change'})).toBeVisible()
    await expect(cyaPage.internalReviewCard.getByRole('link', {name: 'Remove'})).toBeVisible()

    // Remove external audit section
    await cyaPage.internalReviewCard.getByRole('link', {name: 'Remove'}).click()
